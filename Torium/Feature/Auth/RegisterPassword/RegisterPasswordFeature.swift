...
@Reducer
struct RegisterPasswordFeature {
    @ObservableState
    ...
    enum Action: BindableAction {
        ...
        case nextTapped
        case nextResponse(Result<User, Error>)
        ...
        Reduce { state, action in
            ...
            case .nextTapped:
                ...
                return .run { [email = state.email, password = state.password] send in
                    await send(
                        .nextResponse(Result{ try await authClient.register(email, password) })
                    )
                }
            ...
            case .nextResponse(.failure(let error)):
                state.isLoading = false
                if let verifyError = error as? RegisterPasswordError,
                   verifyError == .emailAlreadyRegistered && verifyError == .emailNotVerified {
